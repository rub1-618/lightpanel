...
fn proceed_cmd(command: &mut Command, first_message: String, second_emessage: String, third_emessage: String) -> Result<(), LpnlError> {
        ...
        Ok(status) => {
            if status.success() {
                ...
                return Err(LpnlError::SetupError { 
                    ...
                    kind: SetupErrorKind::InvalidCmdResult
                })
            }
        }
        Err(e) => return Err(LpnlError::SetupError { 
...
fn proceed_nginx() -> Result<(), LpnlError> {
